...
class PriceHistoryService:
    ...
    async def list(self, market_hash_name: str, since: datetime) -> Sequence[PriceHistoryRecordORM]:
        return await self.price_history_repo.list(market_hash_name, since)

    async def add(self, price_record: PriceHistoryRecordCreate) -> PriceHistoryRecord:
        return await self.price_history_repo.add(price_record)

    async def add_many(self, price_records: List[PriceHistoryRecordCreate]) -> List[PriceHistoryRecord]:
        return await self.price_history_repo.add_many(price_records)

    async def delete_before(self, market_hash_name: str, before_date: datetime) -> int:
        return await self.price_history_repo.delete_before(market_hash_name, before_date)
