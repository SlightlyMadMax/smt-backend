...
class TestPriceHistoryService:
    ...
    async def test_list_calls_repo_with_correct_params(
        ...
        mock_price_history_repo.list.return_value = filtered_records

        # Act
        result = await price_history_service.list(market_hash_name, sample_datetime)

        # Assert
        mock_price_history_repo.list.assert_called_once_with(market_hash_name, sample_datetime)
    ...
    async def test_list_returns_empty_sequence_when_no_records(
        ...
        mock_price_history_repo.list.return_value = []
    ...
    async def test_list_handles_different_datetime_filters(
        ...
        mock_price_history_repo.list.return_value = expected_filtered

        # Act
        result = await price_history_service.list(market_hash_name, filter_datetime)

        # Assert
        mock_price_history_repo.list.assert_called_once_with(market_hash_name, filter_datetime)
    ...
    async def test_add_one_calls_repo_with_correct_params(
        ...
        # Arrange
        mock_price_history_repo.add.return_value = sample_price_record_orm

        # Act
        result = await price_history_service.add(sample_price_record_create)

        # Assert
        mock_price_history_repo.add.assert_called_once_with(sample_price_record_create)
    ...
    async def test_add_one_handles_none_return_from_repo(
        ...
        mock_price_history_repo.add.return_value = None
    ...
    async def test_add_one_with_different_price_values(self, price_history_service, mock_price_history_repo):
        ...
        mock_price_history_repo.add.return_value = expected_orm

        # Act
        result = await price_history_service.add(price_record)

        # Assert
        mock_price_history_repo.add.assert_called_once_with(price_record)
    ...
    async def test_add_many_calls_repo_with_correct_params(
        ...
        # Arrange
        mock_price_history_repo.add_many.return_value = sample_price_records_orm

        # Act
        result = await price_history_service.add_many(sample_price_records_create)

        # Assert
        mock_price_history_repo.add_many.assert_called_once_with(sample_price_records_create)
    ...
    async def test_add_many_handles_empty_list(self, price_history_service, mock_price_history_repo):
        # Arrange
        empty_list = []
        mock_price_history_repo.add_many.return_value = []

        # Act
        result = await price_history_service.add_many(empty_list)

        # Assert
        mock_price_history_repo.add_many.assert_called_once_with(empty_list)
        assert result == []
    ...
    async def test_add_many_handles_single_record(
        ...
        mock_price_history_repo.add_many.return_value = expected_orm_list

        # Act
        result = await price_history_service.add_many(single_record_list)

        # Assert
        mock_price_history_repo.add_many.assert_called_once_with(single_record_list)
        assert len(result) == 1
    ...
    async def test_add_many_handles_partial_success(
        ...
        mock_price_history_repo.add_many.return_value = partial_success_orm
    ...
    async def test_add_many_preserves_order_and_data(self, price_history_service, mock_price_history_repo):
        ...
        mock_price_history_repo.add_many.return_value = expected_orm
    ...
    async def test_list_propagates_repo_exception(
        ...
        # Arrange
        mock_price_history_repo.list.side_effect = Exception("Database error")
    ...
    async def test_add_one_propagates_repo_exception(
        ...
        # Arrange
        mock_price_history_repo.add.side_effect = Exception("Database error")

        # Act & Assert
        with pytest.raises(Exception, match="Database error"):
            await price_history_service.add(sample_price_record_create)

    async def test_add_many_propagates_repo_exception(
        self, price_history_service, mock_price_history_repo, sample_price_records_create
    ):
        # Arrange
        mock_price_history_repo.add_many.side_effect = Exception("Database error")
    ...
    async def test_list_handles_special_characters_in_market_hash_name(
        ...
        # Assert
        mock_price_history_repo.list.assert_called_once_with(special_name, sample_datetime)
        assert result == []
    ...
    async def test_add_one_handles_special_characters_in_market_hash_name(
        ...
        # Act
        result = await price_history_service.add(special_record)

        # Assert
        mock_price_history_repo.add.assert_called_once_with(special_record)
